        ...
        <table width="623px" border="0" cellpadding="0" cellspacing="0">
            <tr>
                <td align="justify">
                    ...
                    <p>
                        Por medio del presente certifico que {{ strGenderPrintstroa($voluntario->genero)}} {{ strtoupper($voluntario->Apellidos) }} {{ strtoupper($voluntario->apellidoMaterno) }} {{ strtoupper($voluntario->Nombres) }} {{ strtoupper($voluntario->nombreSegundo) }}, portador  de la Cédula de Identidad {{ $voluntario->Pasaporte }}, estudiante de la carrera de {{ $periodo === null ? $voluntario->Carrera : $periodo->carrera }} de la {{ $periodo === null ? $voluntario->universidad->Nombre : $periodo->universidad->Nombre }}, realizó PRACTICAS {{ $practica }} en {{ $periodo === null ? $voluntario->unidad->Nombre : $periodo->unidad->Nombre }}, institución regentada por la Benemérita Sociedad Protectora de la Infancia, en base al Convenio de Cooperación Interinstitucional entre la {{ $periodo === null ? $voluntario->universidad->Nombre : $periodo->universidad->Nombre }} y la Benemérita Sociedad Protectora de la Infancia firmado el {{ $firma_fecha }} desde el {{ $periodo === null ? formatDateComplete($voluntario->FechaInicio) : formatDateComplete($periodo->fecha_inicio) }} al {{ $periodo === null ? formatDateComplete($voluntario->FechaFin) : formatDateComplete($periodo->fecha_fin) }} en el Departamento {{ $periodo === null ? $voluntario->departamento->Nombre : $periodo->departamento->Nombre }},, completando un total de {{ $periodo === null ? $voluntario->HorasProgramada : $periodo->horas_programada }} horas. 
                    </p>
